...
class Upscale1d(Module):
    ...
    def __init__(self, scale_factor, zero_insert=False):
        super(Upscale1d, self).__init__()
        self.scale_factor = scale_factor
        self.zero_insert = zero_insert

    def forward(self, x, **kwargs):
        input_shape = x.size()
        tmp = x.view(input_shape[0], input_shape[1],
                     input_shape[2], 1)
...
class CubicUpsampling1d(Module):
    ...
    def forward(self, x, **kwargs):
        old_size = x.size()
        x = F.pad(x, pad=[2, 2], mode='replicate')
        # output = input
        weight = Variable(self.kernel.expand(old_size[1], -1, -1), requires_grad=False).contiguous()
        output = F.conv_transpose1d(x, weight, groups=old_size[1], stride=self.scale_factor)
